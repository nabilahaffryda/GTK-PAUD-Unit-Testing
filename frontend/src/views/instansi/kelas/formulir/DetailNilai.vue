          ...
          <v-card flat>
            <v-simple-table>
              <template v-slot:default>
                ...
                <tbody>
                  <tr
                    v-for="(ins, i) in instruments.filter(
                      (instrument) =>
                        Number(
                          $getDeepObj(instrument, 'm_instrumen_nilai_luring_paud.data.k_tahap_nilai_luring_paud') || 0
                        ) === Number(tab + 1)
                    )"
                    :key="i"
                  >
...
export default {
  ...
  methods: {
    getTotal(tahap) {
      let result = 0;
      const intrument = this.instruments.filter(
        (instrument) =>
          Number(this.$getDeepObj(instrument, 'm_instrumen_nilai_luring_paud.data.k_tahap_nilai_luring_paud') || 0) ===
          Number(tahap)
      );
